...
Doc.prototype._handleOp = function(err, message) {
  ...
  if (this.data.$schema !== message.sv && !message.frc) {
    return this.emit('error', new ShareDBError(4027, 'Op from the future'));
  }
...
Doc.prototype.submitOp = function(component, options, callback) {
  ...
  // Append the client schemaVersion to the top level op
  if (options && options.sv) {
    op.sv = options.sv
  }
  // Specify whether or not this is a client upgrade migration op
  if (options && options.mig) {
    op.mig = options.mig
  }
  // Specify if we are forcing this op regardless of the schema
  if( options && options.frc) {
    op.frc = options.frc
  }
