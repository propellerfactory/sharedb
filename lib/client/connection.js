...
Connection.prototype.bindToSocket = function(socket) {
  ...
  socket.onmessage = function(event) {
    try {
      ...
      util.log('warn', 'Failed to parse message', { event })
      return;
    }

    if (connection.debug) console.log('RECV', JSON.stringify(data));
    util.log('info', 'sharedb connection RECV', { data: JSON.stringify(data) })
...
Connection.prototype._handleBulkMessage = function(message, method) {
  if (message.data) {
    ...
    util.log('error', 'Invalid bulk message', { message })
  }
};
...
Connection.prototype.send = function(message) {
  ...
  util.log('info', 'sharedb connection SEND', { message: JSON.stringify(message) })
