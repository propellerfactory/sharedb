...
SubmitRequest.prototype.submit = function(callback) {
  ...
  backend.db.getSnapshot(collection, id, fields, null, function(err, snapshot) {
    ...
    // if this is a new document the snapshot is empty and we dont care so we just set the data
    if (snapshot.data) {
      const opSchemaVersion = op.sv || 0
      const snapshotSchemaVersion = snapshot.data.$schema || 0
      if (op.mig) {
        if (opSchemaVersion <= snapshotSchemaVersion) {
          return callback({code: 4026, message: 'Migration is to an older version'})
        }
      } else if (opSchemaVersion !== snapshotSchemaVersion && !op.frc) {
        return callback({code: 4025, message: 'Incorrect schema version'})
      }
    }
...
SubmitRequest.prototype.commit = function(callback) {
  ...
  backend.trigger('commit', this.agent, this, function(err) {
    ...
    backend.db.commit(request.collection, request.id, request.op, request.snapshot, request.options, function(err, succeeded) {
      if (err || !succeeded) {
        util.log('debug', 'sharedb commit op failed', { err: err, succeeded: succeeded })
...
SubmitRequest.prototype.retry = function(callback, err) {
  ...
  if (this.maxRetries != null && this.retries > this.maxRetries) {
    util.log('debug', 'sharedb max retries exceeded', { retry: this.retries })
    return callback(this.maxRetriesError(err));
  }

  util.log('debug', 'sharedb retrying submit op', { retry: this.retries })
