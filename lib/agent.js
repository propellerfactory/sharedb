...
Agent.prototype.close = function(err) {
  if (err) {
    util.log('warn', 'Agent closed due to error', {
      clientId: this.clientId, stack: err.stack || err
    })
  }
...
Agent.prototype._reply = function(request, err, message) {
  if (err) {
    if (typeof err === 'string') {
      ...
      util.log('error', 'sharedb agent replying with error', {
        message: err
      })
    } else {
      if (err.stack) {
...
function CreateOp(src, seq, v, create, sv, mig, frc) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.create = create;
...
function EditOp(src, seq, v, op, sv, mig, frc) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.op = op;
...
function DeleteOp(src, seq, v, del, sv, mig, frc) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.del = del;
...
Agent.prototype._createOp = function(request) {
  ...
  if (request.op) {
    return new EditOp(src, request.seq, request.v, request.op, request.sv, request.mig, request.frc);
  } else if (request.create) {
    return new CreateOp(src, request.seq, request.v, request.create, request.sv, request.mig, request.frc);
  } else if (request.del) {
    return new DeleteOp(src, request.seq, request.v, request.del, request.sv, request.mig, request.frc);
  }
};
