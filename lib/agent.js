...
function CreateOp(src, seq, v, create, sv, mig) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.create = create;
...
function EditOp(src, seq, v, op, sv, mig) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.op = op;
...
function DeleteOp(src, seq, v, del, sv, mig) {
  this.src = src;
  this.seq = seq;
  this.v = v;
  this.del = del;
...
Agent.prototype._createOp = function(request) {
  ...
  if (request.op) {
    return new EditOp(src, request.seq, request.v, request.op, request.sv, request.mig);
  } else if (request.create) {
    return new CreateOp(src, request.seq, request.v, request.create, request.sv, request.mig);
  } else if (request.del) {
    return new DeleteOp(src, request.seq, request.v, request.del, request.sv, request.mig);
  }
};
